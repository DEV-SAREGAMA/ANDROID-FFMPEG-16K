...
extern int ffmpeg_android_is_cancel_requested(void);
FILE *vstats_file;
...
{
    // Android-side cancellation (from FFmpegNative.nativeRequestCancel())
    if (ffmpeg_android_is_cancel_requested())
        return AVERROR_EXIT; // or 1, but AVERROR_EXIT is consistent with signals
